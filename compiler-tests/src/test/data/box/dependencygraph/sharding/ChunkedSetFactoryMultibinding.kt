...
/*
 * This test verifies that set multibinding chunking works correctly with the
 * SetFactory path (triggered when set is injected as Provider<Set<T>>).
 * With STATEMENTS_PER_MULTIBIND_FUN=2 and 5 set elements, the addProvider() calls
 * should be split across 3 private helper methods (2+2+1).
 *
 * Validation: All 5 set elements are present via Provider<Set<String>>
 */
...
interface TestGraph {
  ...
  val elements: Provider<Set<String>>
}
